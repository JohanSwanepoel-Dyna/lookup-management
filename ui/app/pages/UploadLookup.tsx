...
function detectDelimiter(lines: string[]): string {
  ...
  for (const delim of candidates) {
    counts[delim] = lines.slice(0, 5).reduce((acc, l) => acc + (l.split(delim).length - 1), 0);
  }
...
function inferDplType(values: string[]): "INT" | "DOUBLE" | "LD" {
  const nonEmpty = values.filter((v) => v.trim() !== "");
  if (nonEmpty.length === 0) return "LD";
  if (nonEmpty.every((v) => /^-?\d+$/.test(v.trim()))) return "INT";
  if (nonEmpty.every((v) => /^-?\d+(\.\d+)?$/.test(v.trim()))) return "DOUBLE";
  return "LD";
}

function buildDplPattern(text: string): { pattern: string; lookupField: string } | null {
  ...
  const stripQuotes = (v: string) => v.replace(/^"(.*)"$/, "$1").trim();

  const headerCells = lines[0].split(delimiter).map(stripQuotes);
  const columnNames = headerCells.map(sanitiseColumnName);

  // Sample up to 10 data rows for type inference
  const dataRows = lines.slice(1, 11).map((l) => l.split(delimiter).map(stripQuotes));

  const tokens = columnNames.map((name, i) => {
    const colValues = dataRows.map((row) => row[i] ?? "");
    const type = inferDplType(colValues);
    return `${type}:${name}`;
  });

  // Build delimiter literal for DPL (e.g. ',' or '\t')
  const delimLiteral = delimiter === "\t" ? "'\\t'" : `'${delimiter}'`;

  const pattern = tokens.join(` ${delimLiteral} `);
  const lookupField = columnNames[0] ?? "";

  return { pattern, lookupField };
}
...
export const UploadLookup = () => {
  ...
  const [dplAutoGenerated, setDplAutoGenerated] = useState(false);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      // Auto-suggest DPL pattern for CSV files
      if (fileFormat === "csv") {
        const suggestion = buildDplPattern(text);
        if (suggestion) {
          setDplPattern(suggestion.pattern);
          setDplAutoGenerated(true);
          if (!lookupField) setLookupField(suggestion.lookupField);
        }
      }
    };
        ...
        <Flex flexDirection="column" padding={24} gap={20}>
          ...
          <Flex flexDirection="column" gap={4}>
            ...
            <TextInput
              ...
              placeholder="e.g. HTTP Status Codes"
            />
            <Text style={{ color: Colors.Text.Neutral.Subdued }}>
              A human-friendly name shown in the lookup list (optional)
            </Text>
          </Flex>
          ...
          <Flex flexDirection="column" gap={4}>
            ...
            <TextInput
              value={dplPattern}
              onChange={(v) => { setDplPattern(v); setDplAutoGenerated(false); }}
              placeholder={
                fileFormat === "jsonl"
                  ? "JSON:json"
                  : fileFormat === "csv"
            ...
            />
            {dplAutoGenerated && (
              <MessageContainer variant="neutral" onDismiss={() => setDplAutoGenerated(false)}>
                <MessageContainer.Title>DPL pattern auto-generated</MessageContainer.Title>
                <MessageContainer.Description>
                  The pattern was derived from your CSV headers. Review it before uploading and adjust if needed.
                </MessageContainer.Description>
              </MessageContainer>
            )}
            <Text style={{ color: Colors.Text.Neutral.Subdued }}>
